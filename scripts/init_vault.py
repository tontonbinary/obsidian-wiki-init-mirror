...
> ⚠️ **分类可自定义，但命名规范必须遵循模板**
> 
> 用户可根据自己的需求调整 raw/ 下的文件夹分类，
> 但文件命名必须遵循「来源简称-标题摘要.md」或「YYYY-MM-DD-类型.md」的格式。
> 命名规范与模板一致，确保 Agent 能正确识别和处理。
...
> ⚠️ **本章节为模板标准，用户一般无需修改，直接使用**

> 核心原则：所有内容必须经过 raw 再编译到 wiki，禁止跳过 raw 直接按记忆写入
...
> ⚠️ **分类可自定义，但页面格式必须遵循模板**
> 
> 用户可根据自己的需求调整 wiki/ 下的文件夹分类和命名，
> 但页面格式（frontmatter 字段、章节结构）必须遵循模板标准。
> 页面格式与模板一致，确保 Agent 能正确解析和编译。
